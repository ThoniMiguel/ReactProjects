...
const Panel = ({img, name, job, text, changeNumber, randomNumber}) => {

    return (
        <article className="review">
            <img src={img} alt="persons face" className="review-img"/>
            <h4 className="author">{name}</h4>
            <p className="job">{job}</p>
            <p className="info">{text}</p>
            <div className="buttons-container">
                <button className="prev-btn" onClick={() => {changeNumber(-1)}}>
                ...
                <button className="next-btn" onClick={() => {changeNumber(1)}}>
